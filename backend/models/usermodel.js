...
async function createArtist(user_id) {
	const query =
		"INSERT INTO artists (user_id, art_types, rating, about) VALUES ($1, $2, $3, $4) RETURNING *";
...
async function getArtistsModel() {
	const query =
		"SELECT users.name, users.profile_picture, artists.art_types, artists.rating , artists.artist_id FROM users JOIN artists ON users.id = artists.user_id WHERE users.user_role = 'artist'";
	return await db.query(query);
}
